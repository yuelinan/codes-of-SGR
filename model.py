...
class Graph_Teacher(torch.nn.Module):
    ...
    def __init__(self, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        super(Graph_Teacher, self).__init__()
...
class Graph_Student_MSE(torch.nn.Module):
    ...
    def __init__(self, args, num_tasks, num_layer = 5, emb_dim = 300, gnn_type = 'gin', drop_ratio = 0.5, gamma = 0.4, use_linear_predictor=False):
        ...
        self.teacher = Graph_Teacher(num_tasks, num_layer, emb_dim, gnn_type, drop_ratio, gamma, use_linear_predictor)
    ...
    def forward(self, batched_data):
        ...
        h_node = self.graph_encoder(batched_data)
        with torch.no_grad():
            teacher_h = self.teacher.eval_forward2(batched_data)
        ...
        if pred_rep.size(-1) != 1:
            ...
            augument_pred = torch.sigmoid(pred_rep).reshape(size,-1)  
            rationale_pred = torch.sigmoid(pred_rem).repeat(1,augument_pred.size(-1))
