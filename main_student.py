...
from utils import init_weights, get_args, eval_test,train_student
...
def main(args):
    ...
    model = eval(args.model_name)(args, gnn_type = args.gnn, num_tasks = dataset.num_tasks, num_layer = args.num_layer,
    ...
    opt_predictor = optim.Adam(list(model.graph_encoder.parameters())+list(model.predictor.parameters()) +list(model.mlp.parameters()) +list(model.infonce.parameters()), lr=args.lr, weight_decay=args.l2reg)
    ...
    for epoch in range(args.epochs):
        ...
        if args.train_type == 'student':
            train_student(args, model, device, train_loader, optimizers, dataset.task_type, optimizer_name,loss_logger,Dmodel,D_optimizer)
